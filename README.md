# eval-do_not_answer_adversarial
...
**Paper:** https://arxiv.org/abs/2308.13387v2

An adversarial extension of the Do-Not-Answer benchmark that wraps each of the 939 harmful
prompts (spanning five risk areas) in one of three jailbreak transforms -- prefix injection,
refusal suppression, or their combination -- drawn from Wei et al. 2023. The model's response
is scored against the original instruction using a seven-action behavioural taxonomy. The
primary metric is follow_rate per risk area, with the delta versus the clean baseline
indicating safeguard degradation under attack.

## At a glance

| | |
|---|---|
| Upstream | [`register/do_not_answer_adversarial`](https://github.com/UKGovernmentBEIS/inspect_evals/tree/main/register/do_not_answer_adversarial) |
| Group | — |
| Total samples | 0 |
...
```bash
inspect eval inspect_evals/do_not_answer_adversarial \
  --sample-id "<sample-id>" \
